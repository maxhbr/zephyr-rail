...
static bt_uuid_128 kSonySvcUuid = BT_UUID_INIT_128(
...
bool SonyRemote::ready() const {
    return conn_ != nullptr && ff01_handle_ != 0;
}
...
void SonyRemote::zoomTRelease()  { send_cmd(ZOOM_T_RELEASE, sizeof(ZOOM_T_RELEASE)); }
void SonyRemote::zoomWPress()    { send_cmd(ZOOM_W_PRESS,   sizeof(ZOOM_W_PRESS)); }
void SonyRemote::zoomWRelease()  { send_cmd(ZOOM_W_RELEASE, sizeof(ZOOM_W_RELEASE)); }
...
void SonyRemote::on_connected(bt_conn* conn, uint8_t err) {
    ...
    printk("Connected\n");
    if (self_->conn_) bt_conn_unref(self_->conn_);
...
uint8_t SonyRemote::on_discover(bt_conn* /*conn*/,
...
void SonyRemote::start_discovery() {
    ...
    if (err) printk("Primary svc discover err: %d (continuing)\n", err);
    ...
    if (err) printk("Char discover err: %d\n", err);
}
...
void SonyRemote::send_cmd(const uint8_t* buf, size_t len) {
    ...
    int err = bt_gatt_write_without_response(conn_, ff01_handle_, buf, len, false);
    if (err) printk("WWR failed: %d\n", err);
}
