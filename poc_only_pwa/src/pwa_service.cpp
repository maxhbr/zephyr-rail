...
ssize_t PwaService::cmdWrite(struct bt_conn *conn,
  ...
  LOG_INF("═══════════════════════════════════════════════════════");
  LOG_INF("PWA Command Received: '%s'", cmd);
  LOG_INF("═══════════════════════════════════════════════════════");
  ...
  if (strcmp(token, "PING") == 0) {
    LOG_INF("→ Command: PING");
    notifyStatus("PONG");
    return len;
  }

  if (strcmp(token, "STATUS") == 0) {
    LOG_INF("→ Command: STATUS");
    notifyStatus("ACK:STATUS");
    return len;
  }

  if (strcmp(token, "HELLO") == 0) {
    LOG_INF("→ Command: HELLO");
    notifyStatus("ACK:HELLO_FROM_MCU");
    return len;
  }
...
void PwaService::init() {
  ...
  LOG_INF("PWA Service initialized - ready for Web Bluetooth connections");
}
...
void PwaService::onDisconnected(struct bt_conn *conn, uint8_t reason) {
  LOG_INF("═══════════════════════════════════════════════════════");
  LOG_INF("PWA Disconnected (reason: %u)", reason);
  LOG_INF("═══════════════════════════════════════════════════════");
